 ...
 */
class PayPal_WP_Button_Manager_Company {
    ...
    public function __construct(){
        $this->signup_url = WBP_API_LINK . 'generate-signup-link';
        $this->ppcp_url = WBP_API_LINK . 'ppcp-request';
        $this->sandbox_merchant_id = WBP_SANDBOX_PARTNER_MERCHANT_ID;
        $this->live_merchant_id = WBP_LIVE_PARTNER_MERCHANT_ID;
        $this->logger = new PayPal_WP_Button_Manager_Logger( 'ppcp-seller-onboard' );
        add_action( 'admin_menu', array( $this, 'admin_menu') );
        add_action('admin_enqueue_scripts', array( $this, 'admin_enqueue_scripts' ) );
        add_action('admin_post_wbp_add_company', array( $this, 'create_company') );
        add_action('admin_post_wbp_added_company', array( $this, 'update_company') );
    ...
    public function admin_menu(){
        $wbp_companies_page = add_submenu_page( 'edit.php?post_type=paypal_button', __('PayPal Companies','paypal-wp-button-manager'), __('PayPal Companies','paypal-wp-button-manager'), 'manage_options', self::$paypal_button_company_slug, array( $this, 'paypal_button_manager_admin') );
        add_action("load-$wbp_companies_page", array( $this, 'companies_screen_options') );
    }

    /**
     * Enqueues the admin style and js
     * */
    public function admin_enqueue_scripts(){
        if( isset( $_GET['page'] ) && $_GET['page'] == self::$paypal_button_company_slug ){
            wp_enqueue_style( 'wbp-company', WBP_PLUGIN_URL . 'assets/backend/css/wbp-company.css', array(), '1.0.0' );
            wp_enqueue_script( 'wbp-company', WBP_PLUGIN_URL . 'assets/backend/js/wbp-company.js', array(), '1.0.0');
        }
    }
    ...
    public function paypal_button_manager_admin(){
        ...
        if( isset( $_GET['company_id'] ) ){
            ...
            if( isset( $company->paypal_merchant_id ) && !empty( $company->paypal_merchant_id ) ){
                ...
                include_once( WBP_PLUGIN_PATH .'/templates/admin/wbp-company-information.php');
            } else {
                $redirect_url = $this->get_signup_url( $company_id, $company->tracking_id, $company->country, $company->paypal_mode );
                
                include_once( WBP_PLUGIN_PATH .'/templates/admin/wbp-company.php');
            }
        } else {
            if( isset( $_GET['type'] ) && $_GET['type'] == 'new' ){
                ...
                if( $companies > 0 ){
                    include_once( WBP_PLUGIN_PATH .'/templates/admin/wbp-company-list.php');
                } else {
                    include_once( WBP_PLUGIN_PATH . '/templates/admin/wbp-add-new-button.php');
                }
            }
        }
    }
    ...
    public function companies_screen_options(){
        $option = 'per_page';
        $args   = [
            'label'   => __('PayPal Companies','paypal-wp-button-manager'),
            'default' => 20,
        ...
        $this->paypal_companies = new PayPal_WP_Button_Manager_companies();
    }
    ...
    private function get_signup_url( $company_id, $tracking_id, $country=null, $mode='live' ){
        ...
        $request_body = array(
            'testmode' => $testmode,
            'return_url' => admin_url('admin-post.php?action=wbp_added_company&company_id=' . $company_id),
            'return_url_description' => __('Return to your shop', 'paypal-wp-button-manager'),
            'products' => $products,
        ...
        return new WP_Error('signup_url_error', __('Unknown Error', 'paypal-wp-button-manager') );
    }
    ...
    private function get_onboarding_status( $merchant_id, $mode='live' ){
        ...
        if( $response['response']['code'] != 200 ){
            ...
            return new WP_Error('seller-onboarding-status-error', __('Internal Server Error', 'paypal-wp-button-manager') );
        }
