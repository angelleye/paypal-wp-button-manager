...
class Angelleye_Paypal_Wp_Button_Manager_Shortcode{
    ...
    public function print_button( $attrs ){
        ...
        if( $button->get_button_type() == 'services' ){
            ...
            wp_localize_script( $this->plugin_name . '-frontend-button', 'btn_obj_' . $button_id, array(
                'hosted_button_id' => $button->get_hosted_button_id(),
                'redirect' => get_site_url() . '/angelleye-order-received',
                'button_environment' => $button->get_button_environment(),
                'type' => $button->get_button_type()
            ));

            wp_enqueue_script( $this->plugin_name . '-paypal-sdk', 'https://www.paypalobjects.com/donate/sdk/donate-sdk.js' , array(), null);
            wp_enqueue_script( $this->plugin_name . "-frontend-button" );

            $admin_error_messages = array();
            if( empty( $button->get_hosted_button_id() ) ){
                $admin_error_messages[] = __('Please insert the  button id.', 'angelleye-paypal-wp-button-manager' );
            }
        }
